...
void five_track()
{
    ...
    TH1F *h8 = new TH1F("h8","invM", 100,0,2);
        ...
        if (ntrk == track)
	{
	    ...
	    Double_t p6[3] = {px[5],py[5],pz[5]};
	    
	    if (TMath::Abs(q[0]+q[1]+q[2]+q[3]+q[4]+q[5]+q[6]) == 0)
            {
	    	 continue;
            }
	    ...
	    for (size_t idx1 = 0; idx1 < massList.size(); idx1++)
		   ...
		   Double_t dist2 = massList[idx2].distance;
		   
		   Double_t pn1[3] = {px[i1],py[i1],pz[i1]};
	           Double_t pn2[3] = {px[j1],py[j1],pz[j1]};
		   Double_t pn3[3] = {px[i2],py[i2],pz[i2]};
	           Double_t pn4[3] = {px[j2],py[j2],pz[j2]};
		   
		   
		   const Double_t threshold = 0.2; 
